...
// current dir relative to package.json
const CURRENT_DIR = "./lib/commands/new/gitignore";
...
module.exports = {
  ...
  showPrompt: function () {
    inquirer
      ...
      .then((answers) => {
        this.fileInfo.addHeader = !this.command.opts().noheader;
          ...
          inquirer
            ...
            .then((a) => {
              ...
              var fromFile = path.resolve(
                path.resolve(CURRENT_DIR) + "/files/" + gitingoreFile
              );
              var MSG = "";
        ...
        if (giToCreate.length) {
          ...
          inquirer
            ...
            .then((a) => {
              ...
              progressbar.show("getting gitignore content from github", progress);
              ...
              for (var i = 0; i < selected.length; i++) {
                ...
                progressbar.show(`getting ${selected[i].filename} from github`, progress);
                ...
                utils.getFromUrl(selected[i].download_url, selected[i])
                  .then((data) => {
                    ...
                    progress = ( selected.length - promises.length ) / selected.length;

                    progressbar.show(`applying ${item.filename} from github to ${file}`, progress);

                    // add filename header
                    contents = `## +++++++++ ${item.filename} +++++++++ \n${contents}\n`;
                    fs.appendFileSync(file, contents);
                    progressbar.show(`${item.filename} Applied to ${file}`, progress);
