...
const COPYRIGHT_REGEX = /^(Copyright)(.*\(c\))?(.*yyyy)?(.*year)?(.*ISC)?([^.\n]+)/gm;

module.exports = {
  commander: null,
  init: function (cmd) {
    this.commander = cmd;
    licenseCmd = this.commander
      .command("license")
    ...
    licenseOpts = utils.getOpts(licenseCmd);
  ...
  createLicense: function (answers) {
    ...
    utils.writeToFile(FILE, licenseTxt.toString(), () => {
      utils.spinner.stop(`\n✅ Created ${logText}`);
      utils.exit();      
    }, (err) => { 
